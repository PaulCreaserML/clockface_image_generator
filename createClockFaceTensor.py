...
def clock_face( diameter, hour, minute, hr_len, min_len, back_shade=255, hand_shade=255, thickness=1 ):
  ...
  x_m = int(diameter/2)
  y_m = int(diameter/2)
  image_add_circle(  np_image, x_m, y_m, int(diameter/2)-5, shade = hand_shade )
  ...
  image_add_tick(    np_image, x_m, y_m, int(diameter/2)-8, step=6, tick_len= 4, shade=140 )
  image = tf.convert_to_tensor( np_image, tf.float32 )
  image = tf.cast( image, dtype=tf.uint8 )
  return image

def clock_face_gen( sub_dir, file_handle, diameter, hour_len, minute_len, thickness=1, invert=0 ):
    base_dir = os.getcwd()
    ...
    if invert == 1:
        ...
        for hour in range(0, 12 ):
            for minute in range( 0, 60):
                ...
                image = clock_face( diameter, hour_precise, minute, hour_len, minute_len, 30, 240,  thickness=thickness )

                filename = "clock_" + str(hour) + "_" + str(minute) + "_" + str(hour_len) + "_" + str(minute_len)+ "_" + str(thickness) + "_1.png"
...
def clock_multiface_gen( dir_name, file_handle, diameter, hand_len_min=70, hand_len_max=90, thickness_max=1, invert=0 ):
  for thickness in range(  1, thickness_max+1):
    for percent in range(  hand_len_min, hand_len_max, 5):
      clock_face_gen( dir_name, file_handle, diameter, int(diameter*percent/300 ), int(diameter*percent/200 ), thickness=thickness,  invert=invert )

def clock_multiface_gen_csv( dir_name, csv_file, diameter, hand_len_min=70, hand_len_max=90, thickness_max=1, invert=0 ):
    ...
    with open( os.path.join( dir_name, csv_file ), 'w') as file_handle:
        ...
        clock_multiface_gen( dir_name, file_handle, diameter, thickness_max=thickness_max, invert=invert )
...
def main( argv ):
    ...
    print(" Directory ", dir  )
    clock_multiface_gen_csv( dir, csv, 144, thickness_max=2, invert=invert )
